...
const sketch = (p: any) => {
  ...
  p.setup = () => {
    ...
    a = p.createInput(-0.54, "number");
    b = p.createInput(0.54, "number");
    btn = p.createButton("redraw");
    btn.mousePressed(() => {
      change = 2;
    });
  };
  ...
  p.draw = () => {
    if (change > 0) {
      p.loadPixels();
      const c = new Complex(parseFloat(a.value()), parseFloat(b.value()));
